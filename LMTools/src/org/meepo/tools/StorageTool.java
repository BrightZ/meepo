...
public class StorageTool {
	...
	public StorageTool(String hdir) {
		this.homeDir = hdir;
	}
	...
	private void iterateSingleDb(Database db) {
		...
		PolymorphismWritable keyEntity = new PolymorphismWritable();
		ObjectId valueEntity = new ObjectId();

		// Distribution valueEntity = new Distribution();
		// PolymorphismWritable storageKey = new PolymorphismWritable(storage);

		Transaction transaction = dbEnv.beginTransaction(null, null);

		Cursor cursor = db.openCursor(transaction, this.cursorConfig);
		...
		try {
			...
			while (cursor.getNext(dbKey, dbValue, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
				...
				Storage storage = (Storage) keyEntity.getObject();
				System.out.println(storage);
				this.writeln(keyEntity.toString());

				if (storage instanceof FTPStorage) {
	...
	public static void main(String args[]) {
		// String hdir = args[0];
		String hdir = "/home/zork/sandbox/bsu";
		StorageTool go = new StorageTool(hdir);
		go.initialize();

		go.iterateSingleDb(go.storageDb);
